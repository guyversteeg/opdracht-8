...
void read_database(ifstream& infile)
{
    assert(true);
    /*Postcondition:
    Filename has been read and file with filename has been opened. */
...
void read_from_file(ifstream& infile)
{
    ...
    //assert(infile);
    /*Postcondition:
    File has been read, saved and has been displayed in the console application.*/
    int counter = 0;
    while(infile)
    {
        infile>>MusicDB[counter];
        TrackDisplay T1={true,true,true,true,true,true,true,true};
        counter++;
    }
    cout<<"The number of tracks is "<<counter<<endl;
}
